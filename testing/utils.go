...
func (u *TestUser) Like(t *testing.T, post *atproto.RepoStrongRef) {
	...
	_, err := atproto.RepoCreateRecord(ctx, u.client, &atproto.RepoCreateRecord_Input{
		...
		Record: &lexutil.LexiconTypeDecoder{Val: &gndr.FeedLike{
			LexiconTypeID: "app.gndr.feed.like",
			CreatedAt:     time.Now().Format(time.RFC3339),
			Subject:       post,
		}},
	})
...
func GenerateFakeRepo(r *repo.Repo, size int) (cid.Cid, error) {
	...
	for i := 0; i < size; i++ {
		switch RandAction() {
		case "post":
			_, _, err := r.CreateRecord(ctx, "app.gndr.feed.post", &gndr.FeedPost{
		...
		case "repost":
			_, _, err := r.CreateRecord(ctx, "app.gndr.feed.repost", &gndr.FeedRepost{
				CreatedAt: time.Now().Format(bsutil.ISO8601),
				Subject: &atproto.RepoStrongRef{
					Uri: RandFakeAtUri("app.gndr.feed.post", ""),
		...
		case "like":
			_, _, err := r.CreateRecord(ctx, "app.gndr.feed.like", &gndr.FeedLike{
				CreatedAt: time.Now().Format(bsutil.ISO8601),
				Subject: &atproto.RepoStrongRef{
					Uri: RandFakeAtUri("app.gndr.feed.post", ""),
					Cid: RandFakeCid().String(),
