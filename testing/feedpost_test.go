...
func TestFeedPostParse(t *testing.T) {
	...
	// this is a post-lex-refactor app.gndr.feed.post record
	...
	assert.Equal("app.gndr.feed.post", fp.LexiconTypeID)
	...
	assert.Equal("app.gndr.embed.recordWithMedia", fp.Embed.EmbedRecordWithMedia.LexiconTypeID)
	...
	assert.Equal(
		&appgndr.EmbedRecordWithMedia{
			LexiconTypeID: "app.gndr.embed.recordWithMedia",
			Media: &appgndr.EmbedRecordWithMedia_Media{
				EmbedImages: &appgndr.EmbedImages{
					LexiconTypeID: "app.gndr.embed.images",
			...
			Record: &appgndr.EmbedRecord{
				LexiconTypeID: "app.gndr.embed.record",
				Record: &comatproto.RepoStrongRef{
					Cid: "bafyreiaku7udekkiijxcuue3sn6esz7qijqj637rigz4xqdw57fk5houji",
					Uri: "at://did:plc:rbtury4cp2sdk4tvnedaqu54/app.gndr.feed.post/3jilislho4s2k",
				},
			},
		},
...
// checks a corner-case with $type: "app.gndr.richtext.facet#link"
func TestFeedPostRichtextLink(t *testing.T) {
	assert := assert.New(t)
	cidBuilder := cid.V1Builder{Codec: 0x71, MhType: 0x12, MhLength: 0}

	// this is a app.gndr.feed.post with richtext link
	...
	assert.Equal("app.gndr.feed.post", recordOrig.LexiconTypeID)
