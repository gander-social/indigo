...
var createFeedGeneratorCmd = &cli.Command{
	...
	Action: func(cctx *cli.Context) error {
		...
		ex, err := atproto.RepoGetRecord(ctx, xrpcc, "", "app.gndr.feed.generator", xrpcc.Auth.Did, rkey)
		if err == nil {
...
var verifyUserCmd = &cli.Command{
	...
	Action: func(cctx *cli.Context) error {
		...
		profrec, err := atproto.RepoGetRecord(ctx, xrpcc, "", "app.gndr.actor.profile", ident.DID.String(), "self")
		if err != nil {
			return err
		}
