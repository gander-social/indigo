		...
		Commands: []*cli.Command{
			...
			&cli.Command{
				...
				Flags: []cli.Flag{
					...
					&cli.StringFlag{
						Name:  "appview",
						Value: "did:web:api.gndr.app#gndr_appview",
...
func getFeed(ctx context.Context, c *client.APIClient) error {
	...
	err := c.Get(ctx, "app.gndr.feed.getAuthorFeed", params, &d)
	if err != nil {
		return err
	}
...
func listRecords(ctx context.Context, c *client.APIClient) error {
	...
	list, err := comatproto.RepoListRecords(ctx, c, "app.gndr.actor.profile", "", 10, "did:plc:ewvi7nxzyoun6zhxrhs64oiz", false)
	if err != nil {
		return err
	}
