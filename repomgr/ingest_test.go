...
func doPost(t *testing.T, cs carstore.CarStore, did string, prev *string, postid int) ([]byte, cid.Cid, string, string) {
	...
	_, tid, err := r.CreateRecord(ctx, "app.gndr.feed.post", &gndr.FeedPost{
...
func TestDuplicateRecord(t *testing.T) {
	...
	p1, _, err := repoman.CreateRecord(ctx, 1, "app.gndr.feed.post", &gndr.FeedPost{
		Text: "hello friend",
	})
	if err != nil {
		t.Fatal(err)
	...
	p2, _, err := repoman.CreateRecord(ctx, 1, "app.gndr.feed.post", &gndr.FeedPost{
		Text: "hello friend",
	})
	if err != nil {
		t.Fatal(err)
	...
	if err := repoman.DeleteRecord(ctx, 1, "app.gndr.feed.post", rkey2); err != nil {
		t.Fatal(err)
	}

	rkey1 := strings.Split(p1, "/")[1]
	c, rec, err := repoman.GetRecord(ctx, 1, "app.gndr.feed.post", rkey1, cid.Undef)
	if err != nil {
		t.Fatal(err)
	}
