	...
	for fname, tf := range backends {
		ot.Run(fname, func(t *testing.T) {
			...
			for i := 0; i < 10; i++ {
				...
				rc, _, err := rr.CreateRecord(ctx, "app.gndr.feed.post", &appgndr.FeedPost{
	...
	for loop := 0; loop < 50; loop++ {
		for i := 0; i < 20; i++ {
			...
			if i%4 == 3 {
				...
				rc, tid, err := rr.CreateRecord(ctx, "app.gndr.feed.post", &appgndr.FeedPost{
					Text: fmt.Sprintf("hey look its a tweet %d", time.Now().UnixNano()),
				})
				if err != nil {
					t.Fatal(err)
				...
				recs = append(recs, rc)
				lastRec = "app.gndr.feed.post/" + tid
			}
...
func setupRepo(ctx context.Context, bs blockstore.Blockstore, mkprofile bool) (cid.Cid, string, error) {
	...
	if mkprofile {
		_, err := nr.PutRecord(ctx, "app.gndr.actor.profile/self", &gndr.ActorProfile{})
		if err != nil {
...
func innerBenchmarkRepoWritesCarstore(b *testing.B, ctx context.Context, cs CarStore, cleanup func(), err error) {
	...
	for i := 0; i < b.N; i++ {
		...
		if _, _, err := rr.CreateRecord(ctx, "app.gndr.feed.post", &appgndr.FeedPost{
	...
	for fname, tf := range backends {
		ot.Run(fname, func(t *testing.T) {
			...
			// explicitly update the profile object
			{
				...
				rc, err := rr.UpdateRecord(ctx, "app.gndr.actor.profile/self", &appgndr.ActorProfile{
					Description: &desc,
				})
				if err != nil {
					t.Fatal(err)
