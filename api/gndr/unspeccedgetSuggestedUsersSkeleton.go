// Code generated by cmd/lexgen (see Makefile's lexgen); DO NOT EDIT.

package gndr

// schema: app.gndr.unspecced.getSuggestedUsersSkeleton

import (
	"context"

	"github.com/gander-social/gander-indigo-sovereign/lex/util"
)

// UnspeccedGetSuggestedUsersSkeleton_Output is the output of a app.gndr.unspecced.getSuggestedUsersSkeleton call.
type UnspeccedGetSuggestedUsersSkeleton_Output struct {
	Dids []string `json:"dids" cborgen:"dids"`
}

// UnspeccedGetSuggestedUsersSkeleton calls the XRPC method "app.gndr.unspecced.getSuggestedUsersSkeleton".
//
// category: Category of users to get suggestions for.
// viewer: DID of the account making the request (not included for public/unauthenticated queries).
func UnspeccedGetSuggestedUsersSkeleton(ctx context.Context, c util.LexClient, category string, limit int64, viewer string) (*UnspeccedGetSuggestedUsersSkeleton_Output, error) {
	var out UnspeccedGetSuggestedUsersSkeleton_Output

	params := map[string]interface{}{}
	if category != "" {
		params["category"] = category
	}
	if limit != 0 {
		params["limit"] = limit
	}
	if viewer != "" {
		params["viewer"] = viewer
	}
	if err := c.LexDo(ctx, util.Query, "", "app.gndr.unspecced.getSuggestedUsersSkeleton", params, nil, &out); err != nil {
		return nil, err
	}

	return &out, nil
}
