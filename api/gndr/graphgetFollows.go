// Code generated by cmd/lexgen (see Makefile's lexgen); DO NOT EDIT.

package gndr

// schema: app.gndr.graph.getFollows

import (
	"context"

	"github.com/gander-social/gander-indigo-sovereign/lex/util"
)

// GraphGetFollows_Output is the output of a app.gndr.graph.getFollows call.
type GraphGetFollows_Output struct {
	Cursor  *string                  `json:"cursor,omitempty" cborgen:"cursor,omitempty"`
	Follows []*ActorDefs_ProfileView `json:"follows" cborgen:"follows"`
	Subject *ActorDefs_ProfileView   `json:"subject" cborgen:"subject"`
}

// GraphGetFollows calls the XRPC method "app.gndr.graph.getFollows".
func GraphGetFollows(ctx context.Context, c util.LexClient, actor string, cursor string, limit int64) (*GraphGetFollows_Output, error) {
	var out GraphGetFollows_Output

	params := map[string]interface{}{}
	params["actor"] = actor
	if cursor != "" {
		params["cursor"] = cursor
	}
	if limit != 0 {
		params["limit"] = limit
	}
	if err := c.LexDo(ctx, util.Query, "", "app.gndr.graph.getFollows", params, nil, &out); err != nil {
		return nil, err
	}

	return &out, nil
}
