...
func init() {
	util.RegisterType("app.gndr.feed.post", &FeedPost{})
} //
// RECORDTYPE: FeedPost
type FeedPost struct {
	LexiconTypeID string `json:"$type,const=app.gndr.feed.post" cborgen:"$type,const=app.gndr.feed.post"`
	...
	// entities: DEPRECATED: replaced by app.gndr.richtext.facet.
...
func (t *FeedPost_Embed) MarshalJSON() ([]byte, error) {
	...
	if t.EmbedRecord != nil {
		t.EmbedRecord.LexiconTypeID = "app.gndr.embed.record"
		return json.Marshal(t.EmbedRecord)
	}
	if t.EmbedRecordWithMedia != nil {
		t.EmbedRecordWithMedia.LexiconTypeID = "app.gndr.embed.recordWithMedia"
...
// FeedPost_Entity is a "entity" in the app.gndr.feed.post schema.
//
...
// FeedPost_ReplyRef is a "replyRef" in the app.gndr.feed.post schema.
...
// FeedPost_TextSlice is a "textSlice" in the app.gndr.feed.post schema.
//
