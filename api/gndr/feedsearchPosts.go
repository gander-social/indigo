...
// schema: app.gndr.feed.searchPosts
...
// FeedSearchPosts_Output is the output of a app.gndr.feed.searchPosts call.
...
// FeedSearchPosts calls the XRPC method "app.gndr.feed.searchPosts".
//
...
func FeedSearchPosts(ctx context.Context, c util.LexClient, author string, cursor string, domain string, lang string, limit int64, mentions string, q string, since string, sort string, tag []string, until string, url string) (*FeedSearchPosts_Output, error) {
	...
	if err := c.LexDo(ctx, util.Query, "", "app.gndr.feed.searchPosts", params, nil, &out); err != nil {
		return nil, err
	}
