...
// schema: app.gndr.graph.searchStarterPacks
...
// GraphSearchStarterPacks_Output is the output of a app.gndr.graph.searchStarterPacks call.
...
// GraphSearchStarterPacks calls the XRPC method "app.gndr.graph.searchStarterPacks".
//
...
func GraphSearchStarterPacks(ctx context.Context, c util.LexClient, cursor string, limit int64, q string) (*GraphSearchStarterPacks_Output, error) {
	...
	params["q"] = q
	if err := c.LexDo(ctx, util.Query, "", "app.gndr.graph.searchStarterPacks", params, nil, &out); err != nil {
		return nil, err
	}
