...
// schema: app.gndr.unspecced.searchPostsSkeleton
...
// UnspeccedSearchPostsSkeleton_Output is the output of a app.gndr.unspecced.searchPostsSkeleton call.
...
// UnspeccedSearchPostsSkeleton calls the XRPC method "app.gndr.unspecced.searchPostsSkeleton".
//
...
func UnspeccedSearchPostsSkeleton(ctx context.Context, c util.LexClient, author string, cursor string, domain string, lang string, limit int64, mentions string, q string, since string, sort string, tag []string, until string, url string, viewer string) (*UnspeccedSearchPostsSkeleton_Output, error) {
	...
	if err := c.LexDo(ctx, util.Query, "", "app.gndr.unspecced.searchPostsSkeleton", params, nil, &out); err != nil {
		return nil, err
	}
