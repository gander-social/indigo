...
// schema: app.gndr.graph.getRelationships
...
// GraphGetRelationships_Output is the output of a app.gndr.graph.getRelationships call.
...
func (t *GraphGetRelationships_Output_Relationships_Elem) MarshalJSON() ([]byte, error) {
	if t.GraphDefs_Relationship != nil {
		t.GraphDefs_Relationship.LexiconTypeID = "app.gndr.graph.defs#relationship"
		return json.Marshal(t.GraphDefs_Relationship)
	}
	if t.GraphDefs_NotFoundActor != nil {
		t.GraphDefs_NotFoundActor.LexiconTypeID = "app.gndr.graph.defs#notFoundActor"
...
// GraphGetRelationships calls the XRPC method "app.gndr.graph.getRelationships".
//
...
func GraphGetRelationships(ctx context.Context, c util.LexClient, actor string, others []string) (*GraphGetRelationships_Output, error) {
	...
	if err := c.LexDo(ctx, util.Query, "", "app.gndr.graph.getRelationships", params, nil, &out); err != nil {
		return nil, err
	}
