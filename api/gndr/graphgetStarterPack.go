...
// schema: app.gndr.graph.getStarterPack
...
// GraphGetStarterPack_Output is the output of a app.gndr.graph.getStarterPack call.
type GraphGetStarterPack_Output struct {
	StarterPack *GraphDefs_StarterPackView `json:"starterPack" cborgen:"starterPack"`
}

// GraphGetStarterPack calls the XRPC method "app.gndr.graph.getStarterPack".
//
...
func GraphGetStarterPack(ctx context.Context, c util.LexClient, starterPack string) (*GraphGetStarterPack_Output, error) {
	...
	if err := c.LexDo(ctx, util.Query, "", "app.gndr.graph.getStarterPack", params, nil, &out); err != nil {
		return nil, err
	}
